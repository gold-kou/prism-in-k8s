...
func init() {
	...
	params.KubeConfig, err = clientcmd.BuildConfigFromFlags("", kubeconfigPath)
	if err != nil {
...
func Run() {
	...
	if isCreate {
		...
		err = k8s.CreateK8sResources(ctx, params.KubeConfig, params.ResourceName, params.NamespaceName)
		if err != nil {
			panic(err)
		}

		err = istio.CreateIstioResources(ctx, params.KubeConfig, params.ResourceName, params.NamespaceName)
		if err != nil {
			panic(err)
		}
		log.Println("[INFO] All resources for prism mock are created successfully")
	} else if isDelete {
		err := istio.DeleteIstioResources(ctx, params.KubeConfig, params.ResourceName, params.NamespaceName)
		if err != nil {
			panic(err)
		}

		err = k8s.DeleteK8sResources(ctx, params.KubeConfig, params.ResourceName, params.NamespaceName)
		if err != nil {
			panic(err)
		}
